...
auto memory_diff(const Memory& orig_memory, const Memory& new_memory) {
	...
	for (auto address = static_cast<size_t>(0); address < Memory::ArrayElements; ++address) {
		...
		if (orig_value != new_value) {
			result.push_back({static_cast<uint16_t>(address), orig_value, new_value});
		}
	}
	return result;
}
...
auto main(int argc, const char** argv) -> int {
	...
	auto break_points = std::vector<uint16_t>{0xc7f1, 0xc7f9, 0xc24f, 0xc252}; // TODO: Compare with bgb, registers look ok to 0xc7f1 (including), memory not checked, diff at 0xc7f1
