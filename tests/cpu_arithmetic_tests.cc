...
TEST_CASE("DEC B - 0x05", "[arithmetic]") {
	...
	SECTION("Three to zero") {
		...
		auto correct_F = MakeFlags{.Z=0, .N=1, .H=0, .C=1}.get() + F_low_nibbel;
		auto correct_registers = MakeRegisters{.F=correct_F, .B=0x02, .PC=0x01}.get();
		CHECK_THAT(cpu.registers(), RegistersCompare{correct_registers});

		cycles = cpu.execute_next();
		CHECK(cycles == 1);
		correct_F = MakeFlags{.Z=0, .N=1, .H=0, .C=1}.get() + F_low_nibbel;
		correct_registers = MakeRegisters{.F=correct_F, .B=0x01, .PC=0x02}.get();
		CHECK_THAT(cpu.registers(), RegistersCompare{correct_registers});

		cycles = cpu.execute_next();
		CHECK(cycles == 1);
		correct_F = MakeFlags{.Z=1, .N=1, .H=0, .C=1}.get() + F_low_nibbel;
	...
	SECTION("Half-carry") {
		...
		const auto correct_F = MakeFlags{.Z=0, .N=1, .H=1, .C=1}.get() + F_low_nibbel;
